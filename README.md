...
## Сохранение модели для того, чтобы можно было ей воспользоваться в боте после обучения
Для того, чтобы можно было использовать модель на пк без видеокарты nvidia то есть без поддержки cuda необходимо вначале перевести веса под CPU и скачать этот вариант, он будет работать на любом ПК.
...
## Вывод
Обученную модель можно очень легко использовать в разных сценариях обернув её в api интерфейс, например с помощью fastapi. Есть очень много сценариев, как можно работать с текстовыми данными. Например при помощи этой системы можно экономить очень много денег на услугах модерации чатов
